...
uint32_t CalcCRC32(const uint8_t* data, int count)
{
    ...
    for (int i = 0; i < count; i++)
    {
        crc = (crc << 8) ^ table32[(crc >> 24) ^ data[i]];
    }
    return crc;
}
...
//小端模式crc32,计算结果与硬件crc32相同,table用0x4c11db7生成
